...
public static class PutTrailImageEndpoint
{
    ...
    public static async Task<Results<Created, ValidationProblem>> Invoke(Guid trailId, [FromForm] IFormFile file, HttpContext context, ITrailRepository repository, FileValidator validator, CancellationToken cancellationToken = default)
    {
        ...
        await repository.UpdateImage(trailImage, cancellationToken);
    ...
    private static async Task SaveImage(IFormFile file, string fileName, CancellationToken cancellationToken = default)
    {
